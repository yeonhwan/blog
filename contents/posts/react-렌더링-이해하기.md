...
<details>
  <summary>목차</summary>
    ...
    <a href="#jsx-와-vdom">Reconciliation</a>
    <a href="#jsx-와-vdom">Fiber 와 더블버퍼링</a>
    <a href="#jsx-와-vdom">Render 와 Commit</a>
...
이렇게 만들어진 `React Element Tree` 는 실제로 화면에 표시하기 위한 UI 요소의 정보를 담은 가벼운 버전의 `DOM` 이라고 할 수 있다.
React 는 이렇게 생성된 `React Element Tree` 를 UI를 화면에 렌더링 하기 위해 사용한다.

그러나 React 가 `React Element Tree`를 통해서 `DOM` 을 직접 조작하는 것은 아니다.
React는 DOM을 직접 조작함으로써 발생하는 리플로우 및 리페인트를 효과적으로 줄이고 효율적인 화면의 업데이트를 보장하기 위해서
생성된 `React Element Tree` 를 사용한다.

_브라우저의 리플로우 리페인트를 말하면서 VDOM 사용의 이유를 좀 더 명확히 하기_

React 는 생성된 `React Element Tree` 에 `DOM` 에 반영되어야 할 UI 요소들 변경사항들을 기록하고 미리 적용시킨다.
즉 실제 `DOM` 조작을 직접 진행에 부수효과를 발생 시키는 것이 아니라, `React Element Tree` 에 새로운 변경사항들을 기록해놓은 뒤 모든 업데이트 및 효과들의 적용이 끝난 다음
한 번에 `DOM` 트리를 변경 사항이 적용된 `React Element Tree` 로 교체하는 방식으로 렌더링 과정을 효과적으로 처리한다.
...
React 가 `DOM` 의 변경사항을 `VDOM` 을 통해 브라우저의 부수효과를 최소한으로 관리해 효율적인 렌더링 과정을 달성하는 것처럼
React 내부에서도 `VDOM` 의 변경사항을 적용하고 `VDOM` 의 상태의 변경을 비교화는 효과적인 방법을 재조정 과정을 통해 달성한다.

과거 `재조정자(Reconciliator)` 는 내부적으로 스택 기반 알고리즘을 사용해 `VDOM` 의 변경사항을 추적하고,
해당 업데이트 내역들을 시간 순서대로 pop 한 뒤 트리를 비교하여 필요한 변경사항들을 반영해나가는 과정을 진행했다.

그러나 이러한 스택 기반 알고리즘은 각각의 렌더링의 순서를 업데이트가 요청된 순서로밖에 지정할 수 밖에 없다는 단점이 있었고,
렌더링 프로세스가 한 번 진행되면 중간에 중단할 수 없었다는 단점이 있었다.
...
이러한 문제를 해결해 사용성을 더 높이기 위해서 React 는 `React Fiber` 라는 새로운 구조와 `Concurrent Rendering` 이라는
시스템을 도입하기로 결정한다.

## Fiber 와 Rendering Phase 
...
이로써 React 의 내부적인 렌더링 프로세스를 파악할 수 있게 되었다.

## Concurrent Rendering

이제 `Concurrent Rendering` 이 무엇인지 이해할 준비가 완료되었다.
